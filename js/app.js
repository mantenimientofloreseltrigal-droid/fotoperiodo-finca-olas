...
function planSemanaActual() {
  ...
  return PLAN.filter(p => p.semana === sem);
}
...
function intentarLogin() {
  ...
  const sem = semanaActual();
  document.getElementById('hdr-sem').textContent = 'Sem '+sem.slice(0,4)+'-'+sem.slice(4);
...
function buildInicio() {
  ...
  document.getElementById('plan-sem-lbl').textContent =
    'Semana '+sem.slice(0,4)+'-'+sem.slice(4)+' · '+planSem.length+' variedades';
}
...
function abrirBloque(b) {
  ...
  document.getElementById('d-lado-a').textContent = info.ladoA;
  document.getElementById('d-lado-b').textContent = info.ladoB;
  // Contar guirnaldas encendidas
  let enc=0, al=0;
  ...
  horos.forEach(h=>{
    for(let n=h.nIni;n<=h.nFin;n++){
      ['A','B'].forEach(lado=>{
        for(let g=1;g<=2;g++){
          const k=gKey(b,n,lado,g);
          const est=estadoGuirnalda(guirnaldas[k]);
          if(est==='encendida') enc++;
  ...
  document.getElementById('d-horos').textContent = horos.length;
...
function renderPlanBloque() {
  ...
  let html = '';
  html += '<div class="plan-week"><span class="plan-week-lbl">Semana actual</span><span class="plan-week-val">'+sem.slice(0,4)+'-W'+sem.slice(4)+'</span></div>';
...
function renderPlanGlobal(){
  const sem=semanaActual();
  const plan=planSemanaActual();
  let html='<div class="plan-week"><span class="plan-week-lbl">Semana actual</span><span class="plan-week-val">'+sem.slice(0,4)+'-W'+sem.slice(4)+'</span></div>';
