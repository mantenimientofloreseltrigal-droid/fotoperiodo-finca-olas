...
function renderGuirnaldas(b) {
  ...
  horos.forEach(h => {
    ...
    for (let n = h.nIni; n <= h.nFin; n++) {
      ...
      // Columna A
      let colA = '';
      colA += `<div onclick="abrirGuirnalda('${kA1}',${b},${n},'A',1)" style="cursor:pointer;margin-bottom:8px">`;
      colA += renderCamaBox(gdA1, 1, cA1a, 'A');
      colA += `<div style="opacity:${estA1==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estA1) + `</div>`;
      colA += renderCamaBox(gdA1, 2, cA1b, 'A');
      colA += `</div>`;
      colA += `<div onclick="abrirGuirnalda('${kA2}',${b},${n},'A',2)" style="cursor:pointer">`;
      colA += renderCamaBox(gdA2, 1, cA2a, 'A');
      colA += `<div style="opacity:${estA2==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estA2) + `</div>`;
      colA += renderCamaBox(gdA2, 2, cA2b, 'A');
      colA += `</div>`;

      // Columna B
      let colB = '';
      colB += `<div onclick="abrirGuirnalda('${kB1}',${b},${n},'B',1)" style="cursor:pointer;margin-bottom:8px">`;
      colB += renderCamaBox(gdB1, 1, cB1a, 'B');
      colB += `<div style="opacity:${estB1==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estB1) + `</div>`;
      colB += renderCamaBox(gdB1, 2, cB1b, 'B');
      colB += `</div>`;
      colB += `<div onclick="abrirGuirnalda('${kB2}',${b},${n},'B',2)" style="cursor:pointer">`;
      colB += renderCamaBox(gdB2, 1, cB2a, 'B');
      colB += `<div style="opacity:${estB2==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estB2) + `</div>`;
      colB += renderCamaBox(gdB2, 2, cB2b, 'B');
      colB += `</div>`;
      ...
      html += `<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:10px;margin-bottom:8px">
        ...
        <div style="display:flex;gap:8px;align-items:flex-start">
          <div style="flex:1;min-width:0">${colA}</div>
          <div style="width:1px;background:rgba(255,255,255,.1);align-self:stretch;flex-shrink:0"></div>
          <div style="flex:1;min-width:0">${colB}</div>
        </div>
      </div>`;
    }
