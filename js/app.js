...
function sembrarCamaIndividual(k, bl, nave, lado, g, numCama) {
  ...
  // VARIEDAD — solo plan de la semana
  ...
  if (semPlan.length) {
    ...
    html += '<option value="">Selecciona variedad del plan...</option>';
    semPlan.forEach(p => {
      html += `<option value="${p.variedad}" data-noches="${p.noches}">${p.variedad} (${p.noches}n · ${p.cantidad}c)</option>`;
    });
    html += '</select>';
  } else {
    html += `<div class="banner bw" style="margin-bottom:6px"><span>📋</span>
      <span>No hay variedades en el plan de esta semana. Usa el botón de abajo para buscar en el catálogo.</span></div>`;
    html += `<select id="ind-var" style="display:none"></select>`;
  }
  html += '</div>';

  // BOTÓN — catálogo completo + nueva variedad
  html += `<button class="btn-outline" style="margin-bottom:12px;border-color:var(--a);color:var(--a)"
    onclick="abrirCatalogoCompleto('${k}',${numCama})">
...
function abrirCatalogoCompleto(k, numCama) {
  ...
  let html = '<div class="modal-handle"></div>';
  html += '<div class="modal-title" style="color:var(--v)">🔍 Buscar variedad</div>';
  html += `<div style="font-size:12px;color:${esNoche?'rgba(255,255,255,.6)':'#666'};margin-bottom:12px">
    Catálogo completo — ${Object.keys(VARIEDADES).length} variedades</div>`;

  // Buscador
  html += `<input type="text" id="cat-search" placeholder="Escribe para filtrar..."
    oninput="filtrarCatalogo()"
    style="width:100%;padding:12px 14px;border:2px solid ${borInp};border-radius:10px;
           font-size:14px;background:${bgInp};color:${colTxt};margin-bottom:10px">`;

  // Lista
  html += `<div id="cat-lista" style="max-height:320px;overflow-y:auto;border:1px solid ${borInp};border-radius:10px">`;
  Object.entries(VARIEDADES).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([v,n]) => {
    if (n === 0) return; // omitir sin noches
    html += `<div class="cat-item" data-nombre="${v.toLowerCase()}" data-noches="${n}"
      onclick="seleccionarDelCatalogo('${k}',${numCama},'${v.replace(/'/g,"\\'")}',${n})"
      style="padding:11px 14px;border-bottom:1px solid ${borInp};cursor:pointer;
             display:flex;justify-content:space-between;align-items:center">
      <span style="font-size:13px;font-weight:600;color:${colTxt}">${v}</span>
      <span style="font-size:12px;font-weight:700;color:var(--vm);flex-shrink:0;margin-left:8px">${n}n</span>
    </div>`;
  });
  html += '</div>';

  // Botón agregar variedad nueva
  html += `<button onclick="abrirAgregarVariedad('${k}',${numCama})"
    style="margin-top:10px;width:100%;padding:11px;border-radius:12px;font-size:13px;
           font-weight:700;cursor:pointer;background:transparent;
           color:var(--a);border:2px solid var(--a)">
    ➕ Agregar variedad nueva al catálogo
  </button>`;
  html += `<button onclick="cerrarModal()"
    style="margin-top:8px;width:100%;padding:11px;border-radius:12px;font-size:13px;
           font-weight:700;cursor:pointer;background:transparent;
           color:${esNoche?'rgba(255,255,255,.5)':'#888'};
           border:2px solid ${esNoche?'rgba(255,255,255,.2)':'#ddd'}">
    Cancelar
  </button>`;
  ...
  document.getElementById('modal-body').style.background = esNoche ? '#1e293b' : '#ffffff';
  document.getElementById('modal-overlay').classList.add('show');
  setTimeout(()=>document.getElementById('cat-search')?.focus(), 200);
}
...
function seleccionarDelCatalogo(k, numCama, variedad, noches) {
  // Inyectar la variedad seleccionada en el select del formulario
  const sel = document.getElementById('ind-var');
  if (sel) {
...
function abrirCatalogoCompletoAnt() {
  ...
  let html = '<div class="modal-handle"></div>';
  html += '<div class="modal-title" style="color:var(--v)">🔍 Buscar variedad</div>';
  html += `<input type="text" id="cat-search-ant" placeholder="Escribe para filtrar..."
    oninput="filtrarCatalogoAnt()"
    style="width:100%;padding:12px 14px;border:2px solid ${borInp};border-radius:10px;
           font-size:14px;background:${bgInp};color:${colTxt};margin-bottom:10px">`;
  html += `<div id="cat-lista-ant" style="max-height:340px;overflow-y:auto;border:1px solid ${borInp};border-radius:10px">`;
  Object.entries(VARIEDADES).sort((a,b)=>a[0].localeCompare(b[0])).forEach(([v,n]) => {
    if(n===0) return;
    html += `<div class="cat-item-ant" data-nombre="${v.toLowerCase()}"
      onclick="seleccionarCatalogoAnt('${v.replace(/'/g,"\\'")}',${n})"
      style="padding:11px 14px;border-bottom:1px solid ${borInp};cursor:pointer;
             display:flex;justify-content:space-between;align-items:center">
      <span style="font-size:13px;font-weight:600;color:${colTxt}">${v}</span>
      <span style="font-size:12px;font-weight:700;color:var(--vm)">${n}n</span>
    </div>`;
  });
  html += '</div>';
  html += `<button onclick="cerrarModal()"
    style="margin-top:10px;width:100%;padding:11px;border-radius:12px;font-size:13px;
           font-weight:700;cursor:pointer;background:transparent;
           color:${esNoche?'rgba(255,255,255,.5)':'#888'};
           border:2px solid ${esNoche?'rgba(255,255,255,.2)':'#ddd'}">Cancelar</button>`;

  document.getElementById('modal-body').innerHTML = html;
  document.getElementById('modal-body').style.background = esNoche ? '#1e293b' : '#ffffff';
  document.getElementById('modal-overlay').classList.add('show');
  setTimeout(()=>document.getElementById('cat-search-ant')?.focus(), 200);
}
