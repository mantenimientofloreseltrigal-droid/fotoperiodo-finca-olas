...
async function autoDescargarAlIniciar() {
  try {
    ...
    if (res.ok && res.importadas > 0) {
      console.log(`Auto-descarga: ${res.importadas} registros importados`);
      buildInicio();
      mostrarToastSync(res.importadas); // reusar el toast de sync
    }
  } catch(e) {
    console.warn('Auto-descarga falló:', e.message);
  }
}
