...
function colorCama(gd, numCama) {
  // Retorna color de indicador según estado de la cama
  const var1 = numCama===1 ? gd?.variedad1 : gd?.variedad2;
  if (!var1) return { bg:'rgba(255,255,255,.06)', borde:'rgba(255,255,255,.12)', txt:'rgba(255,255,255,.3)', label:'Sin sembrar' };
  const est = estadoGuirnalda(gd);
  if (est==='encendida')      return { bg:'rgba(29,158,117,.25)',  borde:'#5DCAA5', txt:'#9FE1CB',  label:'En luces' };
  if (est==='por-vencer')     return { bg:'rgba(245,158,11,.25)',  borde:'#F59E0B', txt:'#fcd34d',  label:'Por vencer' };
  if (est==='apagada-auto')   return { bg:'rgba(226,75,74,.2)',    borde:'#E24B4A', txt:'#fca5a5',  label:'Completada' };
  if (est==='sembrada-apagada') return { bg:'rgba(245,158,11,.15)', borde:'#F59E0B', txt:'#fcd34d', label:'Sembrada' };
  return { bg:'rgba(255,255,255,.06)', borde:'rgba(255,255,255,.12)', txt:'rgba(255,255,255,.3)', label:'Sin sembrar' };
}

function renderCamaBox(gd, numCama, camAbs, lado) {
  const c = colorCama(gd, numCama);
  const varN = numCama===1 ? gd?.variedad1 : gd?.variedad2;
  const align = lado==='A' ? 'left' : 'right';
  return `<div style="background:${c.bg};border:1.5px solid ${c.borde};border-radius:8px;padding:6px 10px;margin-bottom:3px;text-align:${align}">
    <div style="font-size:11px;font-weight:800;color:${c.txt}">Cama ${camAbs}</div>
    ${varN ? `<div style="font-size:9px;color:${c.txt};opacity:.8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${varN}</div>` :
             `<div style="font-size:9px;color:${c.txt}">${c.label}</div>`}
  </div>`;
}
  ...
  horos.forEach(h => {
    html += `<div class="nave-wrap">
      <div class="nave-header">
        <span class="nave-num">🏠 Naves ${h.nIni}–${h.nFin}</span>
        <span class="nave-horo">⏱ ${h.id} · ${h.turno.inicio}–${h.turno.fin}</span>
      </div>`;

    // Encabezado columnas A / B
    html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:4px;padding:0 2px">
      <div style="font-size:10px;font-weight:800;color:rgba(100,200,160,.9);letter-spacing:.08em;text-transform:uppercase">◀ Lado A</div>
      <div style="font-size:10px;font-weight:800;color:rgba(200,120,120,.9);letter-spacing:.08em;text-transform:uppercase;text-align:right">Lado B ▶</div>
    </div>`;

    // Una fila por nave
    for (let n = h.nIni; n <= h.nFin; n++) {
      const camR = camasDeGuirnalda(n, 1); // G1: camas 1-2
      const camR2 = camasDeGuirnalda(n, 2); // G2: camas 3-4

      // Claves de guirnaldas
      const kA1 = gKey(b,n,'A',1), kA2 = gKey(b,n,'A',2);
      const kB1 = gKey(b,n,'B',1), kB2 = gKey(b,n,'B',2);
      const gdA1=guirnaldas[kA1]||{}, gdA2=guirnaldas[kA2]||{};
      const gdB1=guirnaldas[kB1]||{}, gdB2=guirnaldas[kB2]||{};
      const estA1=estadoGuirnalda(gdA1), estA2=estadoGuirnalda(gdA2);
      const estB1=estadoGuirnalda(gdB1), estB2=estadoGuirnalda(gdB2);

      // Camas absolutas
      const base = (n-1)*4;
      const [cA1a,cA1b] = [base+camR[0],  base+camR[1]];
      const [cA2a,cA2b] = [base+camR2[0], base+camR2[1]];
      const [cB1a,cB1b] = [base+camR[0],  base+camR[1]];
      const [cB2a,cB2b] = [base+camR2[0], base+camR2[1]];

      html += `<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:10px;margin-bottom:8px">
        <div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.4);margin-bottom:8px;text-align:center;letter-spacing:.06em">NAVE ${n}</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">`;

      // ── LADO A ──
      html += `<div>`;
      // Guirnalda A1 (camas 1-2)
      html += `<div onclick="abrirGuirnalda('${kA1}',${b},${n},'A',1)" style="cursor:pointer">`;
      html += renderCamaBox(gdA1, 1, cA1a, 'A');
      html += `<div style="opacity:${estA1==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estA1) + `</div>`;
      html += renderCamaBox(gdA1, 2, cA1b, 'A');
      html += `</div>`;
      // Guirnalda A2 (camas 3-4)
      html += `<div onclick="abrirGuirnalda('${kA2}',${b},${n},'A',2)" style="cursor:pointer;margin-top:8px">`;
      html += renderCamaBox(gdA2, 1, cA2a, 'A');
      html += `<div style="opacity:${estA2==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estA2) + `</div>`;
      html += renderCamaBox(gdA2, 2, cA2b, 'A');
      html += `</div>`;
      html += `</div>`;

      // ── SEPARADOR CENTRAL ──
      html += `<div style="width:1px;background:rgba(255,255,255,.08);margin:0 -4px"></div>`;

      // ── LADO B ──
      html += `<div>`;
      // Guirnalda B1 (camas 1-2)
      html += `<div onclick="abrirGuirnalda('${kB1}',${b},${n},'B',1)" style="cursor:pointer">`;
      html += renderCamaBox(gdB1, 1, cB1a, 'B');
      html += `<div style="opacity:${estB1==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estB1) + `</div>`;
      html += renderCamaBox(gdB1, 2, cB1b, 'B');
      html += `</div>`;
      // Guirnalda B2 (camas 3-4)
      html += `<div onclick="abrirGuirnalda('${kB2}',${b},${n},'B',2)" style="cursor:pointer;margin-top:8px">`;
      html += renderCamaBox(gdB2, 1, cB2a, 'B');
      html += `<div style="opacity:${estB2==='sin-sembrar'?.4:1}">` + crearBombillosHTML(estB2) + `</div>`;
      html += renderCamaBox(gdB2, 2, cB2b, 'B');
      html += `</div>`;
      html += `</div>`;

      html += `</div></div>`; // cierre grid + nave
    }
    html += `</div>`; // cierre nave-wrap
  });
