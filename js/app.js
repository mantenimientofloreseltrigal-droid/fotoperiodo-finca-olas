...
function abrirGuirnalda(k, bl, nave, lado, g) {
  ...
  [1,2].forEach(nc => {
    ...
    if(sembN){
      ...
      const valCama = validarCamaDuplicada(k, nc);
...
async function confirmarSiembraIndividual(k, bl, nave, lado, g, numCama) {
  ...
  // VALIDACIÓN ANTI-DUPLICADO
  const validacion = validarCamaDuplicada(k, numCama);
  if (!validacion.ok) {
...
async function guardarSiembraAnterior() {
  ...
  const val_ant    = validarCamaDuplicada(k_ant, numCama_ant);
  if (!val_ant.ok) {
...
async function procesarExcel(input) {
  ...
  try {
    ...
    rows.forEach((row, idx) => {
      ...
      const validMasiva = validarCamaDuplicada(k, numCama);
      if (!validMasiva.ok) {
        errores.push({
