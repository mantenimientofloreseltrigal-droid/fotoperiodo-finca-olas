...
function abrirBloque(b) {
  window._bloqueActivo = b; // guardar bloque activo para re-render
  bloqueAct = b;
  const info = BLOQUES[String(b)];
  if(!info) return;
...
function abrirGuirnalda(k, bl, nave, lado, g) {
  ...
  [1,2].forEach(nc => {
    ...
    if(sembN){
      ...
      // Verificar si la guirnalda permite nueva siembra
      const camBaseV = camaBaseAbsoluta(bl, nave, lado);
      const camAbsV  = camBaseV + camasDeGuirnalda(nave, g)[nc-1];
      const valCama  = validarCamaDuplicada(k, nc, bl, camAbsV, lado);
      ...
      if (valCama.ok) {
        ...
        html += `<div style="margin-top:6px;padding:5px;background:rgba(226,75,74,.1);border-radius:8px;
          font-size:10px;color:#E24B4A;text-align:center;font-weight:700">
          Ciclo activo en guirnalda</div>`;
      }
    }
    html += '</div>';
  });
...
function validarCamaDuplicada(k, numCama) {
  const gd  = guirnaldas[k] || {};
  const est = estadoGuirnalda(gd);

  // Sin siembra previa → OK
  if (est === 'sin-sembrar') return { ok: true };

  // Verificar la cama específica dentro de la guirnalda
  const varCama = numCama === 1 ? gd.variedad1 : gd.variedad2;
  if (!varCama) return { ok: true }; // Esta cama no tiene siembra

  // Ciclo activo o en gracia → BLOQUEAR
  if (est === 'encendida' || est === 'por-vencer' || est === 'apagada-auto') {
    const dr      = diasRest(gd.fechaFin);
    const fIni    = numCama === 1 ? gd.fechaIni1 : gd.fechaIni2;
    const noches  = numCama === 1 ? gd.noches1   : gd.noches2;
    const estado  = est === 'encendida'   ? 'En luces activamente' :
                    est === 'por-vencer'  ? 'Por vencer ('+Math.max(0,dr)+' noches restantes)' :
                    'Ciclo completado — en período de gracia ('+Math.abs(dr)+' días)';
    return {
      ok: false,
      msg: `Esta cama ya tiene una siembra activa.

Variedad: ${varCama}
Fecha siembra: ${fmtF(fIni||gd.fechaIni)}
Noches: ${noches}
Estado: ${estado}

El ciclo debe terminar y expirar antes de sembrar nuevamente.`
    };
  }

  // Expirada → OK para nueva siembra
  return { ok: true };
}
...
async function confirmarSiembraIndividual(k, bl, nave, lado, g, numCama) {
  ...
  // VALIDACIÓN ANTI-DUPLICADO — compara bloque+cama+lado
  const camR_v  = camasDeGuirnalda(nave, g);
  const camAbs1 = camaBaseAbsoluta(bl, nave, lado) + camR_v[numCama-1];
  const validacion = validarCamaDuplicada(k, numCama, bl, camAbs1, lado);
  if (!validacion.ok) {
    // Mostrar error en el modal
    const res = document.getElementById('ind-result');
    if (res) {
      res.style.display = 'block';
      res.innerHTML = `<div style="background:rgba(226,75,74,.1);border:1.5px solid rgba(226,75,74,.3);
        border-radius:10px;padding:12px">
        <div style="font-size:13px;font-weight:700;color:#E24B4A;margin-bottom:6px">
          Siembra bloqueada
        </div>
        <div style="font-size:12px;color:var(--txt);white-space:pre-line;line-height:1.6">
          ${validacion.msg}
        </div>
      </div>`;
    }
...
function showScreen(id){
  ...
  if(!el){ console.error('Screen no encontrada:', id); return; }
...
function irConfig(){
  ...
  setNavSel('nb-config');
  renderConfig(); // async — se actualiza cuando termina
}
...
async function renderConfig() {
  ...
  // Asegurar que la URL esté cargada antes de renderizar
  await cargarSheetsURL();
  let html = '';
  ...
  html += '<div class="card" style="margin-bottom:10px">';
  html += `<div class="banner bb" style="margin-bottom:10px">
    <span>ℹ️</span>
    <span>↑ Subir: envía tus registros a Sheets.<br>↓ Descargar: trae los registros del operario a este celular.</span>
  </div>`;
  html += `<button class="btn-g" style="margin-bottom:8px" onclick="sincronizarManual()">
    ↑ Subir mis registros a Sheets
  </button>`;
  html += `<button class="btn-outline" onclick="descargarOperario()">
    ↓ Descargar registros del operario
  </button>`;
...
async function autoDescargarAlIniciar() {
  try {
    console.log('Iniciando auto-descarga desde Sheets...');
    const res = await descargarDesdSheets();
    console.log('Auto-descarga resultado:', JSON.stringify(res));
    if (res.ok) {
      if (res.importadas > 0) {
        // Forzar re-render completo
        buildInicio();
        // Si hay un bloque abierto, re-renderizar sus guirnaldas
        const bloqueActivo = window._bloqueActivo;
        if (bloqueActivo) {
          renderGuirnaldas(bloqueActivo);
        }
        mostrarToastSync(res.importadas);
        console.log(`✓ ${res.importadas} guirnaldas sincronizadas`);
      } else {
        console.log('Auto-descarga: sin cambios nuevos');
      }
    } else {
      console.warn('Auto-descarga falló:', res.msg);
    }
  } catch(e) {
    console.warn('Auto-descarga error:', e.message);
  }
}

async function descargarOperario() {
  const msg = document.getElementById('msg-sync');
  if (!navigator.onLine) {
    msg.textContent = '⚠ Sin internet. Conéctate al WiFi.';
    msg.className = 'msg err'; msg.style.display = 'block'; return;
  }
  msg.textContent = '↓ Descargando datos del operario...';
  msg.className = 'msg ok'; msg.style.display = 'block';

  const res = await descargarDesdSheets();
  if (res.ok) {
    msg.textContent = `✓ ${res.importadas} registros descargados. Actualiza la vista de bloques.`;
    buildInicio();
    // Refrescar tareas si está en esa pantalla
    if (document.getElementById('sc-tareas')?.classList.contains('show')) {
      renderTareas();
    }
  } else {
...
async function sincronizarManual() {
  const msg = document.getElementById('msg-sync');
  // Intentar cargar URL — usar respaldo directo si es necesario
  if (!SHEETS_URL) await cargarSheetsURL();
  if (!SHEETS_URL && typeof SHEETS_URL_APP !== 'undefined') SHEETS_URL = SHEETS_URL_APP;
  if (!SHEETS_URL) {
...
async function init(){
  ...
  await cargarSheetsURL();
  // Usar respaldo si la URL no cargó
  if (!SHEETS_URL && typeof SHEETS_URL_APP !== 'undefined') SHEETS_URL = SHEETS_URL_APP;
  // Verificar que SHEETS_URL quedó cargado
  console.log('SHEETS_URL cargada:', !!SHEETS_URL, SHEETS_URL ? SHEETS_URL.slice(0,50)+'...' : 'VACÍA');
  initLogin();
}
init();
...
async function guardarSiembraAnterior() {
  ...
  // VALIDACIÓN ANTI-DUPLICADO
  const g_ant      = Math.ceil(posEnNave/2);
  const numCama_ant = posEnNave%2===0 ? 2 : 1;
  const k_ant      = gKey(bl, nave, lado, g_ant);
  const val_ant    = validarCamaDuplicada(k_ant, numCama_ant, parseInt(bl), camAbs, lado);
  if (!val_ant.ok) {
    msg.innerHTML = `<div style="background:rgba(226,75,74,.1);border:1.5px solid rgba(226,75,74,.3);
      border-radius:10px;padding:12px;margin-top:8px">
      <div style="font-size:13px;font-weight:700;color:#E24B4A;margin-bottom:4px">Siembra bloqueada</div>
      <div style="font-size:11px;color:var(--txt);white-space:pre-line;line-height:1.6">${val_ant.msg}</div>
    </div>`;
...
async function procesarExcel(input) {
  ...
  try {
    ...
    rows.forEach((row, idx) => {
      ...
      // VALIDACIÓN ANTI-DUPLICADO en carga masiva
      const validMasiva = validarCamaDuplicada(k, numCama, parseInt(bl), cama, lado);
      if (!validMasiva.ok) {
        errores.push({
          fila, bl, cama, lado, varN,
          errs: ['Cama con ciclo activo — ' +
            (guirnaldas[k]?.['variedad'+numCama]||'sembrada') +
            ' · fin: ' + fmtF(guirnaldas[k]?.fechaFin||'')]
        });
        return;
      }
...
function mostrarResumen(validas, errores) {
  ...
  if(ok.length) {
    ...
    html += `<button onclick="cancelarCargaMasiva()"
      style="width:100%;padding:11px;margin-top:8px;border-radius:12px;
...
function cancelarCargaMasiva() {
  ...
  window._cargaCancelada   = false;
  renderCargaMasiva(); // volver a la pantalla inicial limpia
}
...
async function confirmarCargaMasiva() {
  ...
  // Reemplazar botones con barra de progreso + botón detener
  const wrap = document.getElementById('resumen-carga');
  if(wrap) {
    const progArea = document.createElement('div');
    progArea.id = 'prog-carga';
    progArea.innerHTML = `
      <div style="margin-top:10px">
        <div style="display:flex;justify-content:space-between;font-size:12px;
                    color:var(--ts);margin-bottom:6px">
          <span id="prog-txt">Iniciando...</span>
          <span id="prog-pct">0%</span>
        </div>
        <div style="height:8px;background:var(--g);border-radius:4px;overflow:hidden;margin-bottom:10px">
          <div id="prog-bar" style="height:100%;background:var(--v);border-radius:4px;
               width:0%;transition:width .2s"></div>
        </div>
        <button onclick="detenerCargaMasiva()"
          style="width:100%;padding:10px;border-radius:10px;font-size:13px;font-weight:700;
                 cursor:pointer;background:rgba(226,75,74,.1);color:#E24B4A;
                 border:2px solid rgba(226,75,74,.3)">
          ⏹ Detener carga
        </button>
        <div class="msg" id="msg-carga" style="margin-top:8px"></div>
      </div>`;
    wrap.appendChild(progArea);
    // Ocultar botones anteriores
    wrap.querySelectorAll('button.btn-g, button[onclick="cancelarCargaMasiva()"]')
  ...
  for(const v of datos) {
    ...
    if (window._cargaCancelada) {
      const msg2 = document.getElementById('msg-carga');
      if(msg2){
        msg2.textContent = `⏹ Carga detenida — ${guardados} de ${total} registros guardados.`;
        msg2.className='msg err'; msg2.style.display='block';
      }
      break;
    }
    ...
    if(txt) txt.textContent = `Registrando B${v.bl} · C${v.cama} · L${v.lado}...`;
    ...
    // Enviar a Sheets inmediatamente (no solo a la cola)
    await addToSyncQueue('siembra', gdActual);
    if (navigator.onLine && SHEETS_URL) {
      await enviarASheets('siembra', gdActual).catch(()=>{});
    }
    guardados++;
  }

  // Actualizar progreso a 100%
  const barF = document.getElementById('prog-bar');
  const txtF = document.getElementById('prog-txt');
  const pctF = document.getElementById('prog-pct');
  if(barF) barF.style.width='100%';
  if(pctF) pctF.textContent='100%';
  if(txtF) txtF.textContent='Completado';

  const msgF = document.getElementById('msg-carga');
  if(msgF){
    if(!window._cargaCancelada){
      msgF.textContent = `✓ ${guardados} siembras registradas correctamente.`;
      msgF.className='msg ok';
    }
    msgF.style.display='block';
  }

  // Quitar botón detener
  const btnDet = document.querySelector('[onclick="detenerCargaMasiva()"]');
  if(btnDet) btnDet.style.display='none';

  buildInicio();
  window._cargaMasivaDatos = [];
  window._cargaCancelada   = false;
  // Subir estado completo a Sheets para que otros dispositivos lo vean
  if (navigator.onLine && SHEETS_URL) {
    subirEstadoCompleto();
  }
}
...
// == TAREAS DEL DÍA ==
function irTareas() {
  // Asegurar que la pantalla existe antes de mostrarla
  if (!document.getElementById('sc-tareas')) {
    console.error('sc-tareas no encontrado en DOM');
    irBloques(); return;
  }
...
async function renderTareas() {
  ...
  if (!wrap) { console.error('tareas-content no encontrado'); return; }
  wrap.innerHTML = '<div style="padding:20px;text-align:center;color:var(--ts)">Cargando...</div>';
  try {
  ...
  wrap.innerHTML = html;
    ...
    console.error('Error en renderTareas:', err);
    wrap.innerHTML = `<div style="padding:20px;background:rgba(226,75,74,.1);border-radius:12px;margin:12px">
      <div style="font-size:13px;font-weight:700;color:#E24B4A">Error al cargar tareas</div>
      <div style="font-size:11px;color:var(--ts);margin-top:4px">${err.message}</div>
    </div>`;
  }
}
