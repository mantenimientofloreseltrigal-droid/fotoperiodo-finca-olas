...
function mkCamaBox(gd, numCama, camAbs, lado) {
  ...
  return `<div style="background:${c.bg};border:1.5px solid ${c.borde};border-radius:7px;padding:5px 8px;margin-bottom:3px;text-align:${aln}">` +
    ...
    `<div style="font-size:9px;color:${c.txt};opacity:.8">${varN||'Sin sembrar'}</div></div>`;
}

function mkGuirnalda(k, bl, nave, lado, g, gd, c1, c2) {
  const est = estadoGuirnalda(gd);
  const op  = est==='sin-sembrar' ? '.4' : '1';
  return `<div onclick="abrirGuirnalda('${k}',${bl},${nave},'${lado}',${g})" style="cursor:pointer;margin-bottom:8px">` +
    mkCamaBox(gd, 1, c1, lado) +
    `<div style="opacity:${op}">` + crearBombillosHTML(est) + `</div>` +
    mkCamaBox(gd, 2, c2, lado) +
    `</div>`;
}
...
function renderGuirnaldas(b) {
  ...
  horos.forEach(h => {
    html += `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:10px;margin-bottom:10px">
    ...
    for (let n = h.nIni; n <= h.nFin; n++) {
      ...
      const colA = mkGuirnalda(kA1,b,n,'A',1,gdA1,base+1,base+2) +
                   mkGuirnalda(kA2,b,n,'A',2,gdA2,base+3,base+4);
      const colB = mkGuirnalda(kB1,b,n,'B',1,gdB1,base+1,base+2) +
                   mkGuirnalda(kB2,b,n,'B',2,gdB2,base+3,base+4);
