...
function sembrarCamaIndividual(k, bl, nave, lado, g, numCama) {
  ...
  // VARIEDAD
  html += '<div class="field"><label>Variedad</label>';
  ...
  html += `<button class="btn-outline" style="margin-bottom:12px;border-color:var(--a);color:var(--a)"
    onclick="abrirAgregarVariedad('${k}',${numCama})">
    ➕ Variedad no está en la lista
  </button>`;

  // FECHA DE SIEMBRA
  html += '<div class="field"><label>Fecha de siembra</label>';
  html += `<input type="date" id="ind-fecha" max="${hoy()}" value="${hoy()}"
  ...
  html += `<div style="font-size:10px;color:var(--ts);margin-top:3px">
    Si fue sembrada hoy deja esta fecha. Si fue antes cámbiala.
  </div></div>`;
...
function updateSiembraIndividual(k, numCama) {
  ...
  if (esSiembraAntes) {
    ...
    html += `<div class="rb-row"><span class="rb-key">Noches restantes</span>
      <span class="rb-val" style="color:${nochesRestantes<=3?'var(--r)':'var(--vm)';font-weight:800}">${nochesRestantes}n</span></div>`;
    if (nPerdidas > 0) {
