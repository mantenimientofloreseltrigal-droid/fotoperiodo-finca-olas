...
// ── TIPO DE SIEMBRA: nueva vs anterior ───────────────────────────────────────
let tipoSiembra = 'nueva'; // 'nueva' | 'anterior'

function toggleTipoSiembra(tipo, k, numCama) {
  tipoSiembra = tipo;
  const btnNueva    = document.getElementById('btn-nueva');
  const btnAnterior = document.getElementById('btn-anterior');
  const campoFecha  = document.getElementById('campo-fecha');
  const indFecha    = document.getElementById('ind-fecha');

  if (tipo === 'nueva') {
    btnNueva.style.background    = 'var(--v)';
    btnNueva.style.color         = '#fff';
    btnAnterior.style.background = 'var(--g)';
    btnAnterior.style.color      = 'var(--ts)';
    campoFecha.style.display     = 'none';
    if (indFecha) indFecha.value = '';
  } else {
    btnAnterior.style.background = 'var(--v)';
    btnAnterior.style.color      = '#fff';
    btnNueva.style.background    = 'var(--g)';
    btnNueva.style.color         = 'var(--ts)';
    campoFecha.style.display     = 'block';
    // Establecer fecha de hace una semana como sugerencia
    const hace7 = new Date();
    hace7.setDate(hace7.getDate() - 7);
    if (indFecha) indFecha.value = hace7.toISOString().split('T')[0];
  }
  updateSiembraIndividual(k, numCama);
}
...
function updateSiembraIndividual(k, numCama) {
  ...
  let fechaIniReal = hoy();
  let nochesRestantes = noches;
  let diasTranscurridos = 0;
  let esSiembraAnterior = false;

  // Si es siembra anterior, calcular noches restantes
  if (tipoSiembra === 'anterior') {
    const inpFecha = document.getElementById('ind-fecha');
    const fechaIngresada = inpFecha?.value;
    if (fechaIngresada && fechaIngresada < hoy()) {
      fechaIniReal = fechaIngresada;
      diasTranscurridos = Math.floor((new Date(hoy()) - new Date(fechaIngresada)) / 86400000);
      nochesRestantes = Math.max(0, noches - diasTranscurridos);
      esSiembraAnterior = true;
    }
  }
  ...
  html += `<div class="rb-row"><span class="rb-key">Noches totales</span><span class="rb-val">${noches} noches</span></div>`;

  if (esSiembraAnterior) {
    const colDias = diasTranscurridos >= noches ? 'var(--r)' : 'var(--n)';
    html += `<div class="rb-row"><span class="rb-key">Fecha real siembra</span><span class="rb-val">${fmtF(fechaIniReal)}</span></div>`;
    html += `<div class="rb-row"><span class="rb-key">Días transcurridos</span><span class="rb-val" style="color:${colDias}">${diasTranscurridos} días</span></div>`;
    html += `<div class="rb-row"><span class="rb-key">Noches restantes</span><span class="rb-val" style="color:${nochesRestantes<=3?'var(--r)':'var(--vm)';}">${nochesRestantes} noches</span></div>`;
    if (diasTranscurridos >= noches) {
      html += `<div class="banner ba" style="margin:6px 0"><span>⚠</span><span>Esta cama ya completó su ciclo de luces. Se registrará como apagada automáticamente.</span></div>`;
    }
  }

  html += `<div class="rb-row"><span class="rb-key">Fin esta cama</span><span class="rb-val">${fmtF(estaFin)}</span></div>`;
  if (otraFin) html += `<div class="rb-row"><span class="rb-key">Fin cama adyacente</span><span class="rb-val">${fmtF(otraFin)}</span></div>`;
  html += `<div class="rb-row"><span class="rb-key" style="color:var(--v);font-weight:800">Fecha fin guirnalda</span><span class="rb-val" style="color:var(--v)">${fmtF(finFinal)} (más tardía)</span></div>`;
  html += '<div class="banner bk" style="margin-top:6px"><span>⚡</span><span>Guirnalda se enciende automáticamente.</span></div>';
...
async function confirmarSiembraIndividual(k, bl, nave, lado, g, numCama) {
  ...
  // Determinar fecha real de siembra y noches restantes
  let fechaIniReal    = hoy();
  let nochesRestantes = noches;
  ...
  if (tipoSiembra === 'anterior') {
    const inpFecha = document.getElementById('ind-fecha');
    const fReal    = inpFecha?.value;
    if (fReal && fReal < hoy()) {
      fechaIniReal    = fReal;
      const diasTrans = Math.floor((new Date(hoy()) - new Date(fReal)) / 86400000);
      nochesRestantes = Math.max(0, noches - diasTrans);
      esAnterior      = true;
    }
  }
  ...
  const gdActual = {
    ...
    fechaFin,
    noches: Math.ceil((new Date(fechaFin) - new Date(gd.fechaIni||fechaIniReal)) / 86400000)
  };
...
function renderSiembraAnterior() {
  ...
  // BLOQUE
  html += '<div class="field"><label>Bloque</label><select id="sant-bloque" onchange="actualizarNavesAnt()"><option value="">Selecciona bloque...</option>';
  ...
  // NAVE
  html += '<div class="field"><label>Nave</label><select id="sant-nave" onchange="actualizarCamasAnt()" disabled><option value="">Primero selecciona bloque</option></select></div>';
  ...
  // CAMA
  html += '<div class="field"><label>Cama</label><select id="sant-cama" onchange="calcularSiembraAnt()" disabled><option value="">Primero selecciona nave y lado</option></select></div>';
...
function actualizarNavesAnt() {
  ...
  const sel = document.getElementById('sant-nave');
  if(!bl||!sel) return;
  const info = BLOQUES[bl];
  if(!info) return;
  sel.innerHTML = '<option value="">Selecciona nave...</option>';
  for(let n=1;n<=info.naves;n++) sel.innerHTML+=`<option value="${n}">Nave ${n}</option>`;
  sel.disabled = false;
  actualizarCamasAnt();
}
...
function actualizarCamasAnt() {
  const bl   = document.getElementById('sant-bloque')?.value;
  const nave = parseInt(document.getElementById('sant-nave')?.value||0);
  const lado = document.getElementById('sant-lado')?.value;
  const sel  = document.getElementById('sant-cama');
  if(!sel) return;
  if(!bl||!nave||!lado){ sel.disabled=true; return; }
  const info   = BLOQUES[bl];
  const nCamas = lado==='A'?(info?.navesA?.[nave-1]||info?.cAbase||4):(info?.navesB?.[nave-1]||info?.cBbase||4);
  sel.innerHTML = '<option value="">Selecciona cama...</option>';
  for(let c=1;c<=nCamas;c++){
    const camAbs=(nave-1)*4+c;
    sel.innerHTML+=`<option value="${c}" data-abs="${camAbs}">Cama ${camAbs}</option>`;
  }
  sel.disabled = false;
  calcularSiembraAnt();
}
...
async function guardarSiembraAnterior() {
  ...
  const nave   = parseInt(document.getElementById('sant-nave')?.value||0);
  const lado   = document.getElementById('sant-lado')?.value;
  const camN   = parseInt(document.getElementById('sant-cama')?.value||0);
  const camEl  = document.getElementById('sant-cama');
  const camAbs = parseInt(camEl?.selectedOptions[0]?.dataset?.abs||0);
  const selVar = document.getElementById('sant-var');
  const fReal  = document.getElementById('sant-fecha')?.value;
  const msg    = document.getElementById('msg-sant');
  if(!bl||!nave||!lado||!camN||!selVar?.value||!fReal){
  ...
  const nochRest  = Math.max(0,noches-diasTrans);
  const finCama   = addDias(hoy(),nochRest);
  const g         = Math.ceil(camN/2);
  const numCama   = camN%2===0?2:1;
  const k         = gKey(bl,nave,lado,g);
