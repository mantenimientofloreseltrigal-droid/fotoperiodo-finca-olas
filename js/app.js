...
async function procesarExcel(input) {
  ...
  try {
    ...
    const rows = XLSX.utils.sheet_to_json(ws, { header:1, range:4 }); // desde fila 5 (omitir cabecera+ayuda)
    ...
    rows.forEach((row, idx) => {
      ...
      if(!varN)                 errs.push('Variedad vacía');
      ...
      if(errs.length) {
        errores.push({ fila, bl, cama, lado, varN, errs });
        return;
      }

      // Calcular
      const noches  = VARIEDADES[varN] || 19;
      ...
      validas.push({
        fila, bl:parseInt(bl), cama, lado, varN, noches, fechaStr,
        diasT, nochR, nPerd, finCama, nave, g, numCama, k,
