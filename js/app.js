...
function abrirAgregarVariedad(k, numCama) {
  ...
  html += '<div style="font-size:12px;color:var(--txt2);margin-bottom:14px">Agrega una variedad que no está en el catálogo</div>';
  ...
  html += 'style="padding:12px;border:2px solid var(--bo);border-radius:10px;width:100%;font-size:14px;background:var(--g);color:var(--txt)"></div>';
  html += '<div class="field"><label>Noches de luz requeridas</label>';
  html += '<div style="display:flex;align-items:center;gap:10px">';
  html += '<button onclick="ajustarNuevaVar(-1)" style="width:44px;height:44px;border-radius:50%;border:2px solid var(--bo);background:var(--g);font-size:22px;cursor:pointer;color:var(--r);flex-shrink:0;font-weight:800">−</button>';
  html += '<input type="number" id="nueva-var-noches" value="19" min="1" max="99" ';
  html += 'style="flex:1;padding:12px;border:2px solid var(--bo);border-radius:10px;font-size:22px;font-weight:800;text-align:center;background:var(--g);color:var(--txt)">';
  html += '<button onclick="ajustarNuevaVar(1)" style="width:44px;height:44px;border-radius:50%;border:2px solid var(--bo);background:var(--g);font-size:22px;cursor:pointer;color:var(--vm);flex-shrink:0;font-weight:800">+</button>';
  html += '</div></div>';
  ...
  html += `<button class="btn-outline" style="margin-top:8px" onclick="cerrarModal()">Cancelar</button>`;
  document.getElementById('modal-body').innerHTML = html;
  document.getElementById('modal-overlay').classList.add('show');
