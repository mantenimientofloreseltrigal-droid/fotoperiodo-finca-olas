...
async function descargarDesdSheets() {
  ...
  try {
    // Llamar al Apps Script con acción de lectura
    const url = SHEETS_URL + '?action=descargar&ts=' + Date.now();
    const res = await fetch(url, { method: 'GET' });
    const data = await res.json();

    if (!data.ok) return { ok: false, msg: data.error || 'Error al descargar' };

    // Importar siembras recibidas al IndexedDB local
    let importadas = 0;
    if (data.siembras && data.siembras.length) {
      for (const s of data.siembras) {
        // Reconstruir guirnalda desde los datos del Sheet
        const k = gKey(s.bloque, s.nave, s.lado, s.g);
        const gdExist = guirnaldas[k] || {};

        // Solo actualizar si el registro es más reciente
        if (!gdExist.fechaIni || s.fechaIni > gdExist.fechaIni) {
          const gd = {
            ...gdExist,
            id: k, bloque: parseInt(s.bloque),
            nave: parseInt(s.nave), lado: s.lado,
            g: parseInt(s.g),
            encendida: true,
            fechaIni: s.fechaIni,
            fechaFin: s.fechaFin,
          ...
          if (s.numCama == 1) {
            gd.variedad1 = s.variedad; gd.noches1 = parseInt(s.noches);
            gd.fechaIni1 = s.fechaIni;
          } else {
            gd.variedad2 = s.variedad; gd.noches2 = parseInt(s.noches);
            gd.fechaIni2 = s.fechaIni;
          }
          // Recalcular estado
          const dr = Math.floor((new Date(s.fechaFin) - new Date()) / 86400000);
          gd.encendida = dr >= 0;
